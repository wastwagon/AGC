...
export function NewsArticleShareLinks({ url, title }: NewsArticleShareLinksProps) {
  const encUrl = encodeURIComponent(url);
  const encTitle = encodeURIComponent(title);
  const wa = encodeURIComponent(`${title} ${url}`);
  ...
  const items: { href: string; label: string; node: ReactNode }[] = [
    ...
    },
    {
      ...
      node: <IconWhatsApp className="h-[18px] w-[18px]" />,
    },
    {
