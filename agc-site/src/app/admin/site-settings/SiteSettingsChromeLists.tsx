...
type ThumbEdit = { href: string; alt: string };
...
function cloneThumbs(thumbs: { href: string; alt: string }[]) {
  return thumbs.map((x) => ({ ...x }));
}
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="mt-0.5 text-xs text-slate-600">Image tiles link to these paths; alt text is for accessibility.</p>
          </div>
          <button type="button" className={btn} onClick={() => setThumbRows((prev) => [...prev, { href: "", alt: "" }])}>
            <Plus className="h-4 w-4" />
            Add tile
          </button>
        </div>
            ...
            <li key={i} className="flex flex-wrap items-end gap-2 rounded-lg border border-slate-200 bg-white p-2">
              <div className="grid min-w-0 flex-1 gap-2 sm:grid-cols-2">
                <div>
                  ...
                  <input className={`${input} mt-0.5`} value={row.href} onChange={(e) => setThumbRows((p) => p.map((r, j) => (j === i ? { ...r, href: e.target.value } : r)))} placeholder="/our-work/programs" />
                </div>
                <div>
                  <label className="text-xs font-medium text-slate-600">Image alt text</label>
                  <input className={`${input} mt-0.5`} value={row.alt} onChange={(e) => setThumbRows((p) => p.map((r, j) => (j === i ? { ...r, alt: e.target.value } : r)))} placeholder="Programs" />
                </div>
              </div>
              <button type="button" className={dangerBtn} aria-label="Remove" onClick={() => setThumbRows((p) => p.filter((_, j) => j !== i))}>
                <Trash2 className="h-4 w-4" />
              </button>
            </li>
          ))}
        </ul>
        <button type="button" className="mt-3 text-xs text-slate-500 underline hover:text-slate-700" onClick={() => setThumbRows(DEFAULT_SITE_CHROME.footer.workThumbnails.map((x) => ({ ...x })))}>
          Reset to defaults
        </button>
      </div>
    </div>
