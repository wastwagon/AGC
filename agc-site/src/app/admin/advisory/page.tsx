...
import { deleteAdvisory } from "./actions";
...
export default async function AdminAdvisoryPage() {
  ...
  return (
    <div>
      <AdminPageHeader
        title="Advisory"
        description="Advisory listings for the public Advisory section. Order controls how items appear."
      >
        <Link
          href="/admin/advisory/new"
          ...
          <li key={item.id}>
            <AdminMobileEntityCard
              ...
              actions={
                <>
                  <Link
                    href={`/admin/advisory/${encodeURIComponent(item.id)}/edit`}
                  ...
                  <DeleteButton action={deleteAdvisory.bind(null, item.id)} label="Delete advisory item" />
                </>
              }
            />
          </li>
      ...
      {items.length === 0 && (
        <div className="rounded-xl border border-border bg-white p-12 text-center text-slate-500">
          No advisory items yet. Add your first advisory card.
        </div>
      )}
    </div>
  );
